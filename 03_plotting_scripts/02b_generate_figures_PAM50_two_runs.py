#
# Plot a single subject morphometric metrics (two runs) together with normative values computed from
...
#       -run1 sub-001_ses-01_run-01_T2w_metrics_perslice_PAM50.csv
#       -run2 sub-001_ses-01_run-02_T2w_metrics_perslice_PAM50.csv
...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Plot single subject morphometric metrics (two runs) together with normative values computed "
    ...
    parser.add_argument('-run1', required=True, type=str,
                        help="Path to single-subject CSV file for run 1.")
    parser.add_argument('-run2', required=True, type=str,
                        help="Path to single-subject CSV file for run 2.")
...
def load_single_subject_data(path_single_subject, df_spine_generic_min, df_spine_generic_max):
    ...
    :param path_single_subject: path to single subject CSV file (from run1 or run2)
...
def create_lineplot(df, df_run1, df_run2, number_of_subjects, path_filename, sex=None):
    """
    ...
    Args:
        ...
        df_run1 (pd.dataFrame): dataframe with single subject values from run 1
        df_run2 (pd.dataFrame): dataframe with single subject values from run 2
        number_of_subjects (int): number of subjects in spine-generic dataset
        path_filename (str): path to output figure
    ...
    for index, metric in enumerate(METRICS):
        ...
        sns.lineplot(ax=axs[index], x="Slice (I->S)", y=metric, data=df_run1, linewidth=2, color='green',
                     label=f'run-01')
        # Plot single subject data session 2 in dashed line
        sns.lineplot(ax=axs[index], x="Slice (I->S)", y=metric, data=df_run2, linewidth=2, color='darkorange',
                     linestyle='dashed', label=f'run-02')
        ...
        # Add master title
        plt.suptitle(f'Morphometric measures for run-01 and run-02 in PAM50 template space',
...
def main():
    ...
    df_run1 = load_single_subject_data(path_run1, df_spine_generic_min, df_spine_generic_max)
    df_run2 = load_single_subject_data(path_run2, df_spine_generic_min, df_spine_generic_max)

    participant_id, _ = fetch_participant_and_session(path_run1)
    filename = f'{participant_id}_T2w_lineplot_PAM50.png'
    path_filename = os.path.join(path_out_figures, filename)

    # Check if df_single_subject is not empty, if so, print warning and exit
    if df_run1.empty or df_run2.empty:
    ...
    # Create plots
    create_lineplot(df_normative_data, df_run1, df_run2, number_of_subjects, path_filename,
